...
public class GetTest {
    ...
    public void testSimpleGetOnce() {
        int c = next();
        Supplier<Data> supplier = Get.once(slowGet());
        Collection<Future<Data>> futures = futures(supplier, 20);
        Collection<Data> allData = futures.stream().map(GetTest::await).collect(Collectors.toSet());
    ...
    public void testSimpleGetOptional() {
        int c = next();
        Supplier<Data> supplier = Get.once(slowGet());
        assertTrue(Get.maybeOnce(supplier).get().isEmpty());
        Data v = supplier.get();
        assertTrue(Get.maybeOnce(supplier).get().isPresent());
